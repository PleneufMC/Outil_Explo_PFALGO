...
def _detect_annual_bars(df) -> int:
    """
    V5.4.8 FIX #5: Auto-detect annual bar count from DataFrame index.
...
def compute_metrics(trades: list, initial_capital: float = 100000.0,
    ...
    Parameters:
        ...
        annual_bars: bars per year for annualization.
                     ...
                     V5.4.8 FIX #5: was hardcoded 252*24 which is wrong for D/H4.
    ...
    # V5.4.8 FIX #5: Auto-detect annual_bars from data frequency
...
def passes_oos_gates(metrics: dict, gates: dict = None,
    ...
    """
    Check if metrics pass OOS validation gates.
    ...
    Args:
        metrics: dict with keys 'calmar', 'profit_factor', 'max_dd', 'n_trades'
    ...
    """
    from .config import OOS_GATES, get_oos_dd_limit
    ...
    if metrics.get('calmar', 0) < gates.get('min_calmar', 0.25):
        reasons.append(f"Calmar {metrics['calmar']:.3f} < {gates['min_calmar']}")
        passed = False

    if metrics.get('profit_factor', 0) < gates.get('min_pf', 1.2):
        reasons.append(f"PF {metrics['profit_factor']:.3f} < {gates['min_pf']}")
        passed = False

    if metrics.get('max_dd', 0) < dd_limit:
        reasons.append(f"MaxDD {metrics['max_dd']:.2f}% < {dd_limit}%")
        passed = False

    if metrics.get('n_trades', 0) < gates.get('min_trades', 15):
        reasons.append(f"Trades {metrics['n_trades']} < {gates['min_trades']}")
        passed = False
...
def compute_quality_grade(perf_score: float, robustness_score: float,
    ...
    Returns dict with: quality_score, grade, perf_normalized, robustness_score, flags, multiplier
    """
