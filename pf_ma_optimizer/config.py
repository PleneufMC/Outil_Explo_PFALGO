"""
PF AI Lab 5.4.8 — Configuration & Constants
All parameter ranges, defaults, and gates for the exploration tool.

V5.4.8 CHANGES (Performance Divergence Audit):
...
FOCUSED_SEARCH_SPACE = {
    ...
    # PMax MA mode (how the MA line is built)
    'pmax_ma_mode':      {'type': 'categorical', 'choices': ['classic', 'mult_int', 'mult_dec', 'additive']},
    'pmax_ma_factor':    {'type': 'float', 'low': 0.5, 'high': 2.0, 'step': 0.01, 'condition': "pmax_ma_mode in ('mult_int','mult_dec')"},
    'pmax_ma_distance':  {'type': 'float', 'low': -50.0, 'high': 50.0, 'step': 0.5, 'condition': "pmax_ma_mode == 'additive'"},
    # Independent MA Cross Signal (combined with PMax via AND)
    'use_ma_cross_signal': {'type': 'categorical', 'choices': [True, False]},
    'sig_ma_type':       {'type': 'categorical', 'choices': ['EMA', 'TRIMA'], 'condition': 'use_ma_cross_signal == True'},
    'sig_ma_fast':       {'type': 'int', 'low': 3, 'high': 50, 'condition': 'use_ma_cross_signal == True'},
    'sig_ma_slow':       {'type': 'int', 'low': 10, 'high': 200, 'condition': 'use_ma_cross_signal == True'},
    ...
    # ══════════════════════════════════════════════════════════════════════
    # V5.4.8: Risk Management LOCKED — NOT optimized.
    ...
    # ══════════════════════════════════════════════════════════════════════
    # V5.4.8 FIX #1: VIX/Macro/Partial TP/Session filters REMOVED.
...
FOCUSED_FIXED_PARAMS = {
    'pmax_ma_type': 'EMA',
    # pmax_ma_mode is now in search space (not fixed)
    ...
    'order_pause': 5,
    # ══ V5.4.8: Risk Management LOCKED ("SL Plein" philosophy) ══
    ...
    # ══ V5.4.8 FIX #1: Phantom filters forced OFF ══
    ...
    # ══ V5.4.8 FIX #3: Fib filter explicitly OFF ══
...
FULL_SEARCH_SPACE = {
    **FOCUSED_SEARCH_SPACE,
    ...
    # ── V5.4.8: Inherit remaining search params from Focused ──
...
# ==============================================================================
# V5.4.8 FIX #6: Increased from 10%/5runs to 15%/20runs for better robustness assessment
...
def get_default_config():
    ...
    return {
        ...
        # Risk — V5.4.8: LOCKED ("SL Plein" philosophy)
        ...
        'sl_vol_min_mult': 0.8,
        'sl_vol_max_mult': 1.5,
        # V5.4.8: TP/BE LOCKED (user validates on TV)
        ...
        'long_side': True,
        'short_side': True,
        # V5.3: Partial Profit Taking — V5.4.8: LOCKED OFF
        ...
        # V5.3: Session / Timing Filter — V5.4.8: LOCKED OFF (timezone mismatch risk)
        ...
        'session2_start': 0,
        'session2_end': 0,
        # V5.3: VIX Filter (Security Layer 0) — V5.4.8: LOCKED OFF (no data)
        ...
        # V5.3: Macro Filter — V5.4.8: LOCKED OFF (no data)
        ...
        'use_qq_filter': False,
        'qq_length': 14,
        # Fib range — V5.4.8 FIX #3: explicitly OFF (was contradictory)
