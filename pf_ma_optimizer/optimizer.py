...
def _run_oos_backtest_worker(args):
    ...
    Returns (metrics_dict, compact_trades_list).
    compact_trades_list is a lightweight representation of OOS trades
    suitable for JSON serialisation (<15 KB target).
    """
    config, df_oos, instrument = args
    try:
        trades_oos = run_backtest(df_oos, config, instrument)
        metrics = compute_metrics(trades_oos)

        # Build compact trade list for oos_trades_json
        compact_trades = []
        for t in trades_oos:
            compact_trades.append({
                's': t.get('side', 'long')[0],         # 'l' or 's'
                'ep': round(t.get('entry_price', 0), 5),
                'xp': round(t.get('exit_price', 0), 5),
                'pnl': round(t.get('pnl_pct', 0), 3),
                'dur': t.get('duration_bars', 0),
                'xr': (t.get('exit_reason', '') or '')[:12],
            })
            # Enforce 15 KB size limit: truncate if JSON would exceed it
            if len(compact_trades) > 500:
                break

        return metrics, compact_trades
    except Exception:
...
def run_optimization(df: pd.DataFrame, instrument: str = 'UK100',
    ...
    _empty_oos = ({'n_trades': 0, 'calmar': 0, 'profit_factor': 0, 'max_dd': 0}, [])
        ...
        try:
            ...
            for i, args in enumerate(oos_args):
                try:
                    ...
                    oos_results[i] = _empty_oos
                if progress_callback:
                    progress_callback(n_trials + i + 1, total_progress, _safe_best_value(study))

    top_configs = []
    for i, c in enumerate(oos_candidates):
        oos_metrics, oos_compact_trades = oos_results[i] or _empty_oos
        # FEAT-OOS-ENRICH: compute enriched OOS fields from metrics
        oos_avg_win = oos_metrics.get('avg_win', 0.0)
        oos_avg_loss = oos_metrics.get('avg_loss', 0.0)
        oos_wr = (oos_metrics.get('win_rate', 0) or 0) / 100.0
        oos_expectancy = oos_wr * oos_avg_win + (1 - oos_wr) * oos_avg_loss
        oos_avg_duration = oos_metrics.get('avg_duration_bars', 0)

        # Compact JSON — enforce 15 KB limit
        import json as _json
        oos_trades_blob = _json.dumps(oos_compact_trades, separators=(',', ':'))
        if len(oos_trades_blob) > 15_000:
            # Truncate trade list until under limit
            while len(oos_compact_trades) > 1 and len(oos_trades_blob) > 15_000:
                oos_compact_trades = oos_compact_trades[:len(oos_compact_trades) // 2]
                oos_trades_blob = _json.dumps(oos_compact_trades, separators=(',', ':'))
        ...
        top_configs.append({
            ...
            # FEAT-OOS-ENRICH: new fields
            'oos_avg_win': round(oos_avg_win, 3),
            'oos_avg_loss': round(oos_avg_loss, 3),
            'oos_expectancy': round(oos_expectancy, 4),
            'oos_avg_duration': round(oos_avg_duration, 1),
            'oos_trades_json': oos_trades_blob,
        })
