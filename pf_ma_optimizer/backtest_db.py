...
class BacktestDB:
    ...
    def _init_db(self):
        ...
        with self._conn() as conn:
            ...
            # V5.6 FEAT-OOS-ENRICH: add new OOS metrics columns
            for col_def in [
                ("oos_avg_win", "REAL"),
                ("oos_avg_loss", "REAL"),
                ("oos_expectancy", "REAL"),
                ("oos_avg_duration", "REAL"),
            ]:
                try:
                    conn.execute(f"ALTER TABLE configs ADD COLUMN {col_def[0]} {col_def[1]}")
                except sqlite3.OperationalError:
                    pass  # Column already exists
    ...
    def _insert_config(self, conn: sqlite3.Connection, run_id: int,
        ...
        v5.5: Uses passes_oos_gates() with IS/OOS coherence checks (Phase 1.1).
        """
        from .config import get_oos_dd_limit, OOS_GATES
        from .metrics import passes_oos_gates
        ...
        # v5.5: OOS gate check using passes_oos_gates() with IS/OOS coherence
        oos_pass, _gate_reasons = passes_oos_gates(
            moos, gates=OOS_GATES, instrument=instrument, metrics_is=mis
        )
        ...
        # FEAT-OOS-ENRICH: new OOS metrics from optimizer
        oos_avg_win = _safe_float(cfg_data.get('oos_avg_win', moos.get('avg_win')))
        oos_avg_loss = _safe_float(cfg_data.get('oos_avg_loss', moos.get('avg_loss')))
        oos_expectancy = _safe_float(cfg_data.get('oos_expectancy'))
        oos_avg_duration = _safe_float(cfg_data.get('oos_avg_duration', moos.get('avg_duration_bars')))

        # OOS trades compact JSON:
        # Priority 1: pre-built oos_trades_json string from optimizer (FEAT-OOS-ENRICH)
        # Priority 2: legacy oos_trades_detail list (V5.4)
        oos_trades_json = cfg_data.get('oos_trades_json')
        if oos_trades_json is None:
        ...
        # Enforce 15 KB size limit on oos_trades_json
        if oos_trades_json and len(oos_trades_json.encode('utf-8')) > 15_000:
            try:
                trades_list = json.loads(oos_trades_json)
                if isinstance(trades_list, list):
                    while len(trades_list) > 1 and len(json.dumps(trades_list, separators=(',', ':')).encode('utf-8')) > 15_000:
                        trades_list = trades_list[:len(trades_list) * 2 // 3]
                    oos_trades_json = json.dumps(trades_list, separators=(',', ':'))
            except (json.JSONDecodeError, TypeError):
                oos_trades_json = None
        ...
        conn.execute("""
            INSERT INTO configs (
                ...
                oos_avg_win, oos_avg_loss, oos_expectancy, oos_avg_duration,
                oos_gate_pass, wfe,
        ...
        with self._conn() as conn:
            ...
            for inst in instruments:
                ...
                for r in rows:
                    ...
                    # v5.5: Recompute OOS gate using passes_oos_gates() with
                    # IS/OOS coherence checks (Phase 1.1).
                    # Build IS-like metrics from stored IS columns for ratio check.
                    oos_metrics_dict = {
                        'calmar': d.get('oos_calmar') or 0,
                        'profit_factor': d.get('oos_profit_factor') or 0,
                        'max_dd': d.get('oos_max_dd') or 0,
                        'n_trades': d.get('oos_trades') or 0,
                        'win_rate': d.get('oos_win_rate') or 0,
                    }
                    is_metrics_dict = {
                        'profit_factor': d.get('is_profit_factor') or 0,
                        'win_rate': d.get('is_win_rate') or 0,
                    }
                    from .metrics import passes_oos_gates as _passes_gates
                    gate_pass, _gate_reasons = _passes_gates(
                    ...
                    d['_pf_ratio'] = grade_info.get('pf_ratio')
                    d['_wr_delta'] = grade_info.get('wr_delta')
                    d['_coherence_bonus'] = grade_info.get('coherence_bonus', 0)

                    # ── v5.6: Tier + enhanced analytics ──
                    from .analytics import (oos_gate_tier, enhanced_gate_checks,
                                            degradation_vector, ftmo_readiness)
                    tier_info = oos_gate_tier(d)
                    d['_tier'] = tier_info['tier']
                    d['_tier_score'] = tier_info['tier_score']

                    enh = enhanced_gate_checks(d)
                    d['_enhanced_pass'] = enh['enhanced_pass']
                    d['_rr_ratio'] = enh['rr_ratio']

                    deg = degradation_vector(d)
                    d['_degradation'] = deg['symmetry']
                    d['_avg_degradation'] = deg['avg_degradation']

                    ftmo = ftmo_readiness(d)
                    d['_ftmo_score'] = ftmo['ftmo_score']
                    d['_ftmo_grade'] = ftmo['ftmo_grade']
                ...
                # Sort by tier (GOLD > SILVER > BRONZE > FAIL) then quality descending
                deduped.sort(
                    key=lambda x: (x.get('_tier_score', 0), x['_quality']),
                    reverse=True,
                )
    ...
    def export_full_json(self) -> dict:
        ...
        V2 format (FEAT-OOS-ENRICH) adds: oos_avg_win, oos_avg_loss,
        oos_expectancy, oos_avg_duration to each config row.

        Format:
          {
            "format": "pf_ai_lab_db_v2",
            "exported_at": "2026-03-04T...",
            "runs": [
              {
                <all run columns>,
                "configs": [
                  {<all config columns including new OOS fields>},
                  ...
                ]
              },
              ...
    ...
    def import_full_json(self, data: dict) -> dict:
        ...
        Accepts both v1 and v2 formats (FEAT-OOS-ENRICH).
        For v1 imports, new OOS fields default to NULL via .get().
        ...
        """
        fmt = data.get('format', '')
        if fmt not in ('pf_ai_lab_db_v1', 'pf_ai_lab_db_v2'):
            return {
                'imported_runs': 0, 'imported_configs': 0,
                'skipped_runs': 0,
                'errors': [f"Unknown format: {fmt}. "
                           f"Expected 'pf_ai_lab_db_v1' or 'pf_ai_lab_db_v2'."],
            }
