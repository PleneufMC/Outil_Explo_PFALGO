...
V5.6 GUARDS (Divergence Reduction):
  - pmax_ma_mode forced to 'classic' (only mode in Pine Script V69.3)
  - use_ma_cross_signal forced OFF (doesn't exist in Pine Script V69.3)
  - These guards ensure <25% divergence between AI Lab IS and TradingView results.
...
def run_backtest(df: pd.DataFrame, config: dict,
    ...
    # V5.6 GUARD: Force classic MA mode and disable MA cross signal.
    # Non-classic modes (mult_int, mult_dec, additive) and the independent
    # MA cross signal do NOT exist in Pine Script V69.3 and cause >25%
    # divergence when copying parameters to TradingView.
    ma_mode = config.get('pmax_ma_mode', 'classic')
    if ma_mode != 'classic':
        ma_mode = 'classic'  # Force classic — only mode in Pine Script
    pmax_result = compute_pmax(
        ...
        ma_mode=ma_mode,
        ma_factor=1.0,      # unused in classic mode
        ma_distance=0.0,     # unused in classic mode
    )
    ...
    # V5.6 GUARD: This feature does NOT exist in Pine Script V69.3.
    # It is forced OFF to prevent AI Lab vs TradingView divergence.
    # The use_ma_cross_signal parameter is kept in config for backward
    # compatibility with old DB entries, but ALWAYS ignored at runtime.
    # (Previously: combined with PMax via AND logic when enabled)
