...
**Moteur Python corrige pour P.F.Algo V.69.4 + Interface Web 9 pages + Anti-Overfitting Phase 1 + Analytics v5.6**
...
- **Quality Score v5.6** composite:
  ...
  - IS PF cap: >3.0 x0.80, >4.0 x0.60, >5.0 x0.40
  - IS/OOS coherence bonus: jusqu'a +0.5 pts (PF ratio proche de 1.0)
  - IS Score (tiebreaker): x0.2
  - Anti-overfit flags: OVERFIT_PF_RATIO, IS_PF_TOO_HIGH, OVERFIT_WR_DELTA, IS_OOS_COHERENT
...
# Meilleures configs par instrument (quality score v5.5)
...
| `/audit` | GET | **V5.5** Quality Audit Dashboard (anti-overfit flags, PF Ratio) |
...
### v5.6 / P.F.Algo V.69.4 (9 mars 2026)

**Analytics v5.6 — Quick Wins & Data Enrichment :**
- **Omega Ratio + Tail Ratio** (A1.1, A1.2) : nouveaux ratios dans compute_metrics() capturant la distribution complete des returns et la symetrie des queues
- **OOS Gate Tiers** (A3.1) : classification GOLD / SILVER / BRONZE / FAIL multi-niveaux avec criteres cumulatifs (Calmar, PF, trades, perturbation, WFE)
- **Enhanced Gate** (B1.2, B1.3) : exploitation du ratio avg_win/avg_loss (RR) et de l'expectancy dans la logique de gate (WR < 50% + RR < 1.0 = FAIL)
- **PnL Distribution Analysis** (B2.1) : skewness, kurtosis, outliers 2-sigma, contribution top-5/worst-5
- **Trade Autocorrelation** (B2.2) : detection de clustering des pertes (lag 1-5) avec evaluation du risque
- **Metrics by Exit Reason** (B2.4) : PF, win rate, avg PnL par type de sortie (setup_revers, max_hold_day, etc.)
- **Run-Length Analysis** (B2.5) : series gagnantes/perdantes max et moyennes (critique pour FTMO)
- **FTMO-Readiness Score** (B5.2) : score 0-100 (DD <5%, consecutive loss <=5, profit target 10%, frequence, expectancy)
- **IS/OOS Degradation Vector** (B5.1) : degradation par metrique avec detection de symetrie (UNIFORM / MODERATE / ASYMMETRIC)
- **Long/Short Ratio Warning** (B1.5) : detection de biais directionnel (BALANCED / MODERATE_BIAS / EXTREME_BIAS)
- **Stability Decay Index** (B5.3) : analyse par quartile chronologique avec detection de declin (STABLE / DECLINING / DECAYING)
- **API /api/analysis/<config_id>** : endpoint JSON avec toutes les analyses v5.6 sur une config
- **UI** : colonnes Tier et FTMO dans les pages Audit et Best, badges colores, compteurs par tier
- 52 nouveaux tests v5.6 (344 total), tous passent

**Module analytics.py** : 13 fonctions dans le nouveau module pf_ma_optimizer/analytics.py

### v5.5 / P.F.Algo V.69.4 (9 mars 2026)

**Anti-Overfitting Phase 1 :**
- IS/OOS ratio gates : rejet PF ratio >2.5 ou <0.30, delta win-rate >15pp
- IS PF cap penalties : >3.0 x0.80, >4.0 x0.60, >5.0 x0.40
- Seuils OOS renforces : Calmar >= 0.50, PF >= 1.30, min 25 trades
- Bonus coherence IS/OOS : jusqu'a +0.5 pts pour configs coherentes
- Quality grade flags : OVERFIT_PF_RATIO, IS_PF_TOO_HIGH, OVERFIT_WR_DELTA, IS_OOS_COHERENT
- UI : colonne PF Ratio, modal anti-overfit, flags colores (audit.html, best.html)
- 34 nouveaux tests Phase 1 (117 total), tous passent

**PF_Algo V69.4 (Pine Script V6) :**
- BUG FIX: Macro Score momentum overflow cappe a +/-2.0 par indicateur
- Fonctions modifiees : getRSR_strength(), getRSR_climate(), getRSR_weighted()
- Score total RS/Climate garanti dans [-20, +20] (10 indicateurs x +/-2)
- Le filtre RS Only/Climate Only ne bloque plus silencieusement en marche directionnel

**Installeur :**
- install.sh pour Linux/macOS (venv, deps, start.sh)
- INSTALL.md : guide complet (Windows, Linux, macOS, manuel)
- Version bump 5.5 dans tous les fichiers (install.bat, start.bat, launch.py, etc.)
...
**PF AI Lab 5.4.1 -- Import automatique depuis TradingView (integre dans v5.5) :**
...
- Quality Score v5.6 pour le classement Best Configs (remplace v5.0.7)

---

*PF AI Lab 5.5 | P.F.Algo V.69.4 (Pine Script V6) | PineGuard v1.1*
*"Meme signal, meme barre, meme resultat."*
