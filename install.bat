...
title PF AI Lab 6.0.3 - Installation

:: Change to script directory (fixes admin mode)
cd /d "%~dp0"

echo ============================================================
echo   PF AI Lab 6.0.3 - Installation Windows
echo ============================================================
echo.
echo   Dossier : %CD%
echo.
...
> "%VBS_TEMP%" (
    ...
    echo Shortcut.Description = "PF AI Lab 6.0.3 - Backtest Optimizer"
...
echo.
echo   1. Raccourci bureau    : "PF AI Lab 6.0.3" (icone sur le bureau)
echo   2. Menu Demarrer       : cherchez "PF AI Lab 6.0.3"
...
:: ---- Verify start.bat exists (do NOT overwrite the full version) ----
if not exist "%INSTALL_DIR%\start.bat" (
    echo [WARN] start.bat manquant — creation d une version minimale.
    > "%INSTALL_DIR%\start.bat" (
        echo @echo off
        echo cd /d "%%~dp0"
        echo title PF AI Lab 6.0.3
        echo echo.
        echo echo   PF AI Lab 6.0.3 — Demarrage...
        echo echo   Log : %%CD%%\pf_ai_lab.log
        echo echo.
        echo if not exist "%%CD%%\venv\Scripts\pythonw.exe" (
        echo     if not exist "%%CD%%\venv\Scripts\python.exe" (
        echo         echo [ERREUR] L'environnement virtuel n'existe pas.
        echo         echo Lancez d'abord install.bat pour installer les dependances.
        echo         pause
        echo         exit /b 1
        echo     ^)
        echo ^)
        echo set "PYTHONEXE=%%CD%%\venv\Scripts\pythonw.exe"
        echo if not exist "%%PYTHONEXE%%" set "PYTHONEXE=%%CD%%\venv\Scripts\python.exe"
        echo start "" "%%PYTHONEXE%%" "%%CD%%\launch_desktop.pyw"
        echo echo   Le navigateur va s'ouvrir automatiquement.
        echo timeout /t 8 /nobreak ^>nul
    )
    echo [OK] start.bat cree
) else (
    echo [OK] start.bat existant conserve
)
echo.
