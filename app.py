#!/usr/bin/env python3
"""
PF AI Lab 6.0.3 — Web Dashboard (Flask)
9-page interface for Exploration, Validation, Portfolio, Audit, and Best Configs.

v6.0.3 additions (Risk Management Audit):
  - P0: Export integrity (round-trip validation, instrument reset, SHA-256 checksum, strict mode)
  - P1: Feed discount factor, FTMO-specific metrics, hard constraints, risk bounds
  - P2: Bootstrap CI, PnL percentiles, rolling PF, stress test, overfitting indicator
  - P2: Live Readiness Score (0-100) as primary ranking criterion
  - /api/risk/<config_id> endpoint for full risk analysis
  - /api/export/<config_id> endpoint with integrity validation + metadata

v5.6 additions:
  - Omega Ratio + Tail Ratio in metrics
  - OOS Gate Tiers (GOLD/SILVER/BRONZE/FAIL)
  - FTMO-Readiness Score
  - PnL Distribution Analysis
  - Trade Autocorrelation
  - Stability Decay Index
  - IS/OOS Degradation Vector
  - Long/Short Ratio Warning
  - Enhanced Gate (RR ratio + expectancy)
  - /api/analysis/<config_id> endpoint
"""
...
from pf_ma_optimizer.export_integrity import (
    validate_for_export, export_config_json, compute_config_hash,
    build_export_metadata, ExportValidationError,
)
from pf_ma_optimizer.ftmo_risk import (
    full_risk_analysis, compute_live_readiness_score,
    ftmo_hard_filter, bootstrap_confidence_intervals,
)
...
def api_db_best():
    ...
    V5.6: includes _tier, _ftmo_score, _ftmo_grade, _enhanced_pass, _degradation.
    """
...
def api_db_audit():
    ...
    for c in configs:
        ...
        # v5.5: Use passes_oos_gates() with IS/OOS coherence checks
        oos_metrics_dict = {
            'calmar': oos_calmar_val,
            'profit_factor': oos_pf_val,
            'max_dd': c.get('oos_max_dd') or 0,
            'n_trades': c.get('oos_trades') or 0,
            'win_rate': c.get('oos_win_rate') or 0,
        }
        is_metrics_dict = {
            'profit_factor': c.get('is_profit_factor') or 0,
            'win_rate': c.get('is_win_rate') or 0,
        }
        from pf_ma_optimizer.metrics import passes_oos_gates as _audit_gates
        gate_pass, _gate_reasons = _audit_gates(
            oos_metrics_dict, instrument=inst, metrics_is=is_metrics_dict
        )
        ...
        grade_info = compute_quality_grade(
            ...
            metrics_is=is_metrics_dict,
            metrics_oos=oos_metrics_dict,
        )
        ...
        c['_pf_ratio'] = grade_info.get('pf_ratio')
        c['_wr_delta'] = grade_info.get('wr_delta')
        c['_coherence_bonus'] = grade_info.get('coherence_bonus', 0)
        c['oos_gate_pass'] = int(gate_pass)

        # ── v5.6: Tier + enhanced analytics ──
        from pf_ma_optimizer.analytics import (oos_gate_tier, enhanced_gate_checks,
                                                degradation_vector, ftmo_readiness)
        tier_info = oos_gate_tier(c)
        c['_tier'] = tier_info['tier']
        c['_tier_score'] = tier_info['tier_score']

        enh = enhanced_gate_checks(c)
        c['_enhanced_pass'] = enh['enhanced_pass']
        c['_rr_ratio'] = enh['rr_ratio']

        deg = degradation_vector(c)
        c['_degradation'] = deg['symmetry']
        c['_avg_degradation'] = deg['avg_degradation']

        ftmo = ftmo_readiness(c)
        c['_ftmo_score'] = ftmo['ftmo_score']
        c['_ftmo_grade'] = ftmo['ftmo_grade']
    ...
    # v5.6: tier counts
    tier_counts = {'GOLD': 0, 'SILVER': 0, 'BRONZE': 0, 'FAIL': 0}
    for c in audit_data:
        tier = c.get('_tier', 'FAIL')
...
# ==============================================================================
# v5.6: Full Analysis API Endpoint
# ==============================================================================

@app.route('/api/analysis/<int:config_id>')
def api_analysis(config_id):
    """API: full v5.6 analysis for a single config.

    Returns comprehensive analytics: PnL distribution, autocorrelation,
    exit reasons, run-length, FTMO score, degradation vector, stability decay,
    long/short ratio, tier classification.
    """
    config_row = backtest_db.get_config_by_id(config_id)
    if not config_row:
        return jsonify({'error': f'Config {config_id} not found'}), 404

    from pf_ma_optimizer.analytics import full_v56_analysis
    analysis = full_v56_analysis(config_row)

    return jsonify(_sanitize_for_json({
        'config_id': config_id,
        'instrument': config_row.get('instrument', 'UNKNOWN'),
        'analysis': analysis,
    }))


# ==============================================================================
# v6.0.3: Risk Analysis API Endpoint (P1+P2 audit)
# ==============================================================================

@app.route('/api/risk/<int:config_id>')
def api_risk_analysis(config_id):
    """API: full v6.0.3 risk analysis for a single config.

    Returns comprehensive risk report: feed discount, FTMO metrics,
    hard constraints, risk bounds, bootstrap CI, stress test,
    overfitting indicator, live readiness score.

    Query params:
      account_size (float, default 200000)
      challenge_type (str, default '2-step')
      feed_discount (float, default 0.70)
    """
    config_row = backtest_db.get_config_by_id(config_id)
    if not config_row:
        return jsonify({'error': f'Config {config_id} not found'}), 404

    account_size = request.args.get('account_size', 200000, type=float)
    challenge_type = request.args.get('challenge_type', '2-step')
    feed_discount = request.args.get('feed_discount', None, type=float)

    report = full_risk_analysis(
        config_row,
...
@app.route('/api/export/<int:config_id>')
def api_export_config(config_id):
    """API: export a single config with full integrity validation (P0).

    Returns config + metadata + SHA-256 hash + validation warnings.
    Strict mode blocks export if inconsistencies are detected.

    Query params:
      strict (bool, default 1): refuse export on inconsistencies
      data_source (str, default 'MT5_FTMO')
    """
    config_row = backtest_db.get_config_by_id(config_id)
    if not config_row:
        return jsonify({'error': f'Config {config_id} not found'}), 404

    config_json = config_row.get('config_json')
    if not config_json:
        return jsonify({'error': 'No config JSON available'}), 404
    ...
    instrument = config_row.get('instrument', '')
    strict = request.args.get('strict', '1') == '1'
    data_source = request.args.get('data_source', 'MT5_FTMO')

    try:
        result = export_config_json(
            config, instrument=instrument, strict=strict,
            metadata_kwargs={
                'data_source': data_source,
                'is_period': (config_row.get('is_start'), config_row.get('is_end')),
                'oos_period': (config_row.get('oos_start'), config_row.get('oos_end')),
            }
        )

        # Add risk bounds and live readiness
        from pf_ma_optimizer.ftmo_risk import compute_ftmo_risk_bounds
        result['ftmo_risk_bounds'] = compute_ftmo_risk_bounds(config_row)
        ...
        return jsonify({
            'error': 'Export blocked by strict validation',
            'issues': e.errors,
        }), 422
